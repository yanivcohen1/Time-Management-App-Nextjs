...
function createTodo(text: string): TodoItem {
  return {
    id: typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `todo-${Date.now()}`,
    text,
    completed: false,
    createdAt: Date.now(),
  };
    ...
    <Container size="3" py={{ initial: "5", sm: "7" }}>
      <Flex direction="column" gap="6">
        <Card size="3" variant="surface">
          <Flex direction={{ initial: "column", sm: "row" }} justify="between" align={{ sm: "center" }} gap="4">
        ...
        <Card size="3" variant="surface">
          <Flex direction="column" gap="4">
            <Tabs.Root value={filter} onValueChange={(value) => setFilter(value as FilterValue)}>
                ...
                {FILTERS.map((item) => (
                  <Tabs.Content key={item.value} value={item.value}>
                    <Flex direction="column" gap="3">
                      ...
                      {stats.completed > 0 && (
                        <Flex justify="end">
                          <Button variant="outline" color="gray" size="2" onClick={handleClearCompleted}>
